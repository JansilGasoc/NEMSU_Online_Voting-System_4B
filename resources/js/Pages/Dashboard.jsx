...
import VotingSpinner from '@/Components/VotingSpinner';
import CountdownToEnd from '@/Components/CountdownToEnd';
import { List } from 'lucide-react';

export default function Dashboard({ electionId, candidates = {} }) {
    const { auth } = usePage().props;
    const { election, flash } = usePage().props;

    const [isVotingClosed, setIsVotingClosed] = useState(election?.status === "Paused");

    // Real-time state management
    const [electionStarted, setElectionStarted] = useState(
        election && election.status === 'Started'
    );
    const [realTimeCandidates, setRealTimeCandidates] = useState(candidates);
    ...
    const completeForm = useForm({
        ...
        course_program: auth.user?.course_program || '',
        year: auth.user?.year || '',
    });

    // Form for voting with all positions
    const voteForm = useForm({
        president_id: '',
        internal_vice_president_id: '',
        external_vice_president_id: '',
        secretary_id: '',
        treasurer_id: '',
        auditor_id: '',
        senator_ids: [],
    ...
    useEffect(() => {
        ...
        if (!window.Echo) {
            ...
            window.Echo = new Echo({
                ...
                cluster: "ap1",
                forceTLS: true,
                namespace: "App.Events", // important
                auth: {
                    headers: {
        ...
        const handleElectionStatus = (e) => {
            ...
            if (nextStarted) {
                toast.success("🗳️ Election has started! Please Refresh the page if the form is not appeared", {
                    duration: 20000,
                });
            } else {
                const status = e?.status || "Paused";
                if (status === "Paused") {
                    toast.error("⏸️ Election has been paused.", {
                        duration: 5000,
                    });
    ...
    }, [auth.user?.id, auth.user?.api_token, setElectionStarted]);
    ...
    // Get all position names from real-time candidates
    const positions = Object.keys(realTimeCandidates);
    ...
    const handleExplicitSubmit = (e) => {
        e.preventDefault();
        if (currentPositionIndex === positions.length - 1 && isCurrentPositionValid()) {
            submitVote(e);
        }
    };

    // Check if current position selection is valid
    const isCurrentPositionValid = () => {
        if (!currentPosition) return false;
        if (currentPosition === 'senator') {
            return voteForm.data.senator_ids.length > 0;
        }
        return !!voteForm.data[`${currentPosition}_id`];
    };

    const hasCandidates = Object.keys(realTimeCandidates).length > 0;
    ...
    // PROFILE COMPLETION VIEW (when student_id is empty)
    ...
    // VOTING INTERFACE (when student_id exists)
    return (
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold leading-tight text-gray-800">Choose Deserving Candidates</h2>
                    <div className="flex items-center space-x-2">
                        <div
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${electionStarted ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                }`}
                        >
                            <div
                                className={`w-2 h-2 rounded-full mr-1 ${electionStarted ? 'bg-green-400' : 'bg-red-400'
                                    }`}
                            ></div>
                            ...
                            {hasCandidates ? (
                                <>
                                    <h2 className="text-xl font-bold mb-6 capitalize">
                                        {currentPosition?.replace(/_/g, ' ') || 'positions'}
                                    </h2>

                                    <form onSubmit={handleExplicitSubmit} className="space-y-6">
                                        {/* Position Title */}
                                        <div className="border-b pb-2">
                                            <h3 className="text-xl font-bold text-gray-800 capitalize"></h3>
                                            {currentPosition === 'senator' && (
                                                <p className="text-sm text-gray-500 mt-1">
                                                    Select up to {maxSenators} senators (selected:{' '}
                                                    {voteForm.data.senator_ids.length})
                                                </p>
                                            )}
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {currentPosition === 'senator' ? (
                                                realTimeCandidates[currentPosition]?.map((candidate) => (
                                                    <div
                                                        key={candidate.id}
                                                        className={`border rounded-lg p-4 transition-all ${voteForm.data.senator_ids.includes(candidate.id)
                                                    ...
                                                    >
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                                        {candidate.party_list} party list
                                                                    </span>
                                                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                                        {candidate.year_level} year
                                                                    </span>
                                                                </div>
                                                                <div className="mt-3">
                                                                    <input
                                                                        type="checkbox"
                                                                        id={`senator-${candidate.id}`}
                                                                        checked={voteForm.data.senator_ids.includes(candidate.id)}
                                                                        onChange={() => {
                                                                            const newSenatorIds = voteForm.data.senator_ids.includes(
                                                                                candidate.id
                                                                            )
                                                                                ? voteForm.data.senator_ids.filter(
                                                                                    (id) => id !== candidate.id
                                                                                )
                                                                                : [
                                                                                    ...voteForm.data.senator_ids,
                                                                                    candidate.id,
                                                                                ];
                                                                            voteForm.setData('senator_ids', newSenatorIds);
                                                                        }}
                                                        ...
                                                        <InputError
                                                            message={voteForm.errors?.senator_ids}
                                                ...
                                                realTimeCandidates[currentPosition]?.map((candidate) => (
                                                    <div
                                                        key={candidate.id}
                                                        className={`border rounded-lg p-4 transition-all ${voteForm.data[`${currentPosition}_id`] === candidate.id
                                                            ? 'border-blue-500 bg-blue-50'
                                                            : 'border-gray-200 hover:border-gray-300'
                                                            }`}
                                                    >
                                                        <div className="flex flex-col items-center space-x-4">
                                                            ...
                                                            <div className="flex-1 min-w-0 w-full text-center items-center justify-center">
                                                                ...
                                                                <div className="mt-3">
                                                                    <input
                                                                        type="radio"
                                                                        id={`${currentPosition}-${candidate.id}`}
                                                                        name={currentPosition}
                                                                        checked={
                                                                            voteForm.data[`${currentPosition}_id`] ===
                                                                            candidate.id
                                                                        }
                                                                        onChange={() =>
                                                                            voteForm.setData(`${currentPosition}_id`, candidate.id)
                                                                        }
                                                                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                                                    />
                                                                    <label
                                                                        htmlFor={`${currentPosition}-${candidate.id}`}
                                                                        className="ml-2 text-sm font-medium text-gray-700"
                                                                    >
                                                                        Select
                                                                    </label>
                                        ...
                                        <div className="flex justify-between pt-4 border-t">
                                            <button
                                                ...
                                                className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm ${currentPositionIndex === 0
                                            ...
                                            {currentPositionIndex < positions.length - 1 ? (
                                                <button
                                                    ...
                                                    disabled={!isCurrentPositionValid()}
                                                    className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm ${!isCurrentPositionValid()
                                                        ? 'bg-gray-200 text-gray-500 cursor-not-allowed'
                                                ...
                                                <button
                                                    type="submit"
                                                    disabled={!isCurrentPositionValid() || isLoading}
                                                    className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm ${!isCurrentPositionValid() || isLoading
                                                        ? 'bg-gray-200 text-gray-500 cursor-not-allowed'
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-gray-500">
                                        Please check back later when candidates have been added.
                                    </p>
                                </div>
                            )}
                        </div>
