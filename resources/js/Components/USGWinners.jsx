...
export default function USGWinners({ candidates = [], election }) {
    const printRef = useRef();

    const getWinnersByPosition = (candidates) => {
        const positions = [
            { key: 'president', label: 'President', limit: 1 },
            { key: 'external_vice_president', label: 'External Vice President', limit: 1 },
            { key: 'internal_vice_president', label: 'Internal Vice President', limit: 1 },
            { key: 'secretary', label: 'Secretary', limit: 1 },
            { key: 'treasurer', label: 'Treasurer', limit: 1 },
            { key: 'auditor', label: 'Auditor', limit: 1 },
            { key: 'senator', label: 'Senator', limit: 8 } // ⬅ senators = top 8
        ];

        const winners = {};

        positions.forEach((pos) => {
            const candidatesForPos = candidates.filter(c =>
                (c.position || "").toLowerCase().replace(/\s/g, "_") === pos.key
            );

            const sorted = [...candidatesForPos].sort((a, b) =>
                (b.bsa_votes_count || 0) - (a.bsa_votes_count || 0)
            );

            winners[pos.key] = {
                label: pos.label,
                candidates: sorted.slice(0, pos.limit) // ⬅ NEW: limit winners
            };
        });
    ...
    const handlePrint = () => {
        ...
        // Check if there are any winners to print
        const hasWinners = Object.values(winners).some(data => data.candidates.length > 0);
        if (!hasWinners) {
            alert("No winners data available to print.");
            return;
        }
        ...
        win.document.write(`
            <html>
            <head>
                <title>USG Election Winners</title>
                <style>
                    body { 
                    ...
                    .footer {
                        margin-top: 144px; /* space from table/content */
                        padding: 10px 20px;
                        display: flex;
    ...
    // Check if we have data to enable the button
    const hasWinners = Object.values(winners).some(data => data.candidates.length > 0);
            ...
            <div className="print-wrapper" ref={printRef} style={{ display: "none" }}>
                <div class="content-area">
                    <div className="header">
                        ...
                        <h1>USG {election?.election_name} Winners</h1>
                    </div>
                                    ...
                                    data.candidates.length > 0 ? (
                                        data.candidates.map((cand, idx) => (
                                            <tr key={`${positionKey}-${idx}`}>
                                                {idx === 0 ? (
                                                    <td rowSpan={data.candidates.length}>{data.label}</td>
                                                ) : null}

                                                <td>{cand.name}</td>
                                                <td>{cand.course_program}</td>
                                                <td>{cand.year_level}</td>
                                                <td>{cand.votes || 0}</td>
                                            </tr>
                                        ))
                                    ) : (
                ...
                {/* Horizontal Footer with 3 Images */}
                <div className="footer">
                    ...
                    <div className="footer-right">
                        <img src="/storage/logo/alpas.png" alt="Logo 1" className="footer-image" />
                        <img src="/storage/logo/alpas2.png" alt="Logo 2" className="footer-image" />
                        <img src="/storage/logo/alpas3.png" alt="Logo 3" className="footer-image" />
                    </div>
                </div>
            </div>
        </div>
