...
export default function BsaResults({ candidates = [], election }) {
    ...
    const getWinnersByPosition = (candidates) => {
        const positions = [
            ...
            { key: 'asst_auditor', label: 'Assistant Auditor' },
            { key: 'p_i_o', label: 'Public Information Officer' },
            { key: 'asst_p_i_o', label: 'Assistant Public Information Officer' },
            { key: 'photo_journalist', label: 'Photo Journalist' },
        ];
        const winners = {};

        positions.forEach((pos) => {
            const candidatesForPos = candidates.filter(c => (c.position || "").toLowerCase().replace(/\s/g, "_") === pos.key);
            const sorted = [...candidatesForPos].sort((a, b) => (b.bsa_votes_count || 0) - (a.bsa_votes_count || 0));
    ...
    const handlePrint = () => {
        ...
        win.document.write(`
            <html>
            <head>
                <title>BSA Election Results</title>
                <style>
                    body { 
        ...
        <div>
            ...
            <div ref={printRef} style={{ display: "none" }}>
                <div className="header">
                    ...
                    <h1>BSA {election?.election_name} Winners</h1>
                </div>
                    ...
                    <tbody>
                        {Object.entries(winners).map(([positionKey, data], i) => (
                            <tr key={i}>
                                ...
                                <td>{data.candidates.length > 0 ? data.candidates.reduce((sum, c) => sum + (c.bsa_votes_count || 0), 0) : 0}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
