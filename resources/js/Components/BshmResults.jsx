...
export default function BshmResults({ candidates = [], election }) {
    const printRef = useRef();

    const getWinnersByPosition = (candidates) => {
        const positions = [
            { key: 'governor', label: 'Governor' },
            { key: 'vp_governor_int', label: 'Vice President for Internal' },
            { key: 'vp_governor_ext', label: 'Vice President for External' },
            { key: 'secretary', label: 'Secretary' },
            { key: 'treasurer', label: 'Treasurer' },
            { key: 'auditor', label: 'Auditor' },
            { key: 'p_i_o', label: 'Public Information Officer' },
            { key: 'dept_ambassador', label: 'Department Ambassador' },
            { key: 'dept_ambassadress', label: 'Department Ambassadress' },
            { key: 'sgt_at_arms', label: 'Sergeant at Arms' },
            { key: 'fourth_year_representative', label: 'Fourth Year Representative' },
            { key: 'second_year_representative', label: 'Second Year Representative' }
        ];
        const winners = {};

        positions.forEach((pos) => {
            const candidatesForPos = candidates.filter(c => (c.position || "").toLowerCase().replace(/\s/g, "_") === pos.key);
            const sorted = [...candidatesForPos].sort((a, b) => (b.bshm_votes_count || 0) - (a.bshm_votes_count || 0));
    ...
    const handlePrint = () => {
        ...
        win.document.write(`
            <html>
            <head>
                <title>BSHM Election Results</title>
                <style>
                    body { 
        ...
        <div>
            ...
            <div ref={printRef} style={{ display: "none" }}>
                <div className="header">
                    ...
                    <h1>BSHM {election?.election_name} Winners</h1>
                </div>
                <h1></h1>
                <table>
                    <thead>
                    ...
                    <tbody>
                        {Object.entries(winners).map(([positionKey, data], i) => (
                            <tr key={i}>
                                ...
                                <td>{data.candidates.length > 0 ? data.candidates.reduce((sum, c) => sum + (c.bshm_votes_count || 0), 0) : 0}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
