...
export default function BsbaResults({ candidates = [], election }) {
    const printRef = useRef();

    const getWinnersByPosition = (candidates) => {
        const positions = [
            { key: 'governor', label: 'Governor' },
            { key: 'vp_finance', label: 'Vice President for Finance' },
            { key: 'p_r_o', label: 'Public Relations Officer' },
        ...
        positions.forEach((pos) => {
            ...
            const sorted = [...candidatesForPos].sort((a, b) => (b.bsba_votes_count || 0) - (a.bsba_votes_count || 0));
    ...
    const handlePrint = () => {
        ...
        win.document.write(`
            <html>
            <head>
                <title>BSBA Election Results</title>
                <style>
                    body { 
        ...
        <div>
            ...
            <div ref={printRef} style={{ display: "none" }}>
                <div className="header">
                    ...
                    <h1>BSBA {election?.election_name} Winners</h1>
                </div>
                    ...
                    <tbody>
                        {Object.entries(winners).map(([positionKey, data], i) => (
                            <tr key={i}>
                                ...
                                <td>{data.candidates.length > 0 ? data.candidates.reduce((sum, c) => sum + (c.bsba_votes_count || 0), 0) : 0}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
